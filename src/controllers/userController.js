...
module.exports = {
    ...
    try {
      ...
      if (usuarios.length > 0) {
        usuarios.forEach((user) => {
          if (
            !user.roles.some(function (rol) {
              return rol.nombre === "director" || rol.nombre === "coordinador";
            })
          ) {
            profes.push(user);
          }
  ...
  userbyID: async function (req, res) {
    try {
      let userfound = await User.findById(req.params.id).populate("roles");
      if (!userfound) {
        res.status(400).json({ msg: ["Este usuario no existe"] });
      } else if (userfound.proyectos.length > 0) {
  ...
  asignProyect: async function (req, res) {
    try {
      ...
      const projs = req.body.ids;
      user.proyectos = [];
      projs.forEach((proj) => {
        user.proyectos.push(proj);
      });
      await user.save();
      res.status(200).json({ msg: "Proyecto(s) asignados satisfactoriamente" });
    } catch (err) {
      console.log(err);
      res.status(400).json({ msg: ["Ocurrió un error"] });
    }
